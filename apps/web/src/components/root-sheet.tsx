...
export default function RootSheet({ children, defaultPageTitle }: IProps) {
  const path = usePathname() || ''
  const { isOpen, setIsOpen, pageTitle: pageTitleFromStore } = useSidebarStore()

  const pageTitle = defaultPageTitle || pageTitleFromStore
  ...
  return (
    <>
      <div className="fixed inset-x-0 top-0 z-40 flex h-12 items-center gap-x-6 bg-white px-4 shadow-sm sm:px-6 lg:hidden">
